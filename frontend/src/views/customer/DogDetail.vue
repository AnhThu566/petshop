    ...
    <div class="container custom-container mb-4">
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb bg-transparent p-0 m-0 small">
          ...
          <li class="breadcrumb-item"><router-link to="/dogs" class="text-muted text-decoration-none">Chó cảnh</router-link></li>
          <li class="breadcrumb-item active font-weight-bold text-dark" aria-current="page" v-if="dog">{{ dog.name }}</li>
        </ol>
      </nav>
    </div>
      ...
      <div class="row">
        ...
        <div class="col-lg-4 mb-4 mb-lg-0 pr-lg-4">
          <div class="main-image-box mb-3 p-1" style="border: 2px solid #e1d5ed; border-radius: 8px;">
            <img :src="dog.image ? 'http://localhost:3000' + dog.image : 'https://via.placeholder.com/600x800'" 
                 class="img-fluid w-100 object-fit-cover rounded" style="max-height: 550px;" alt="Hình thú cưng">
          </div>
          <div class="row px-1">
            <div class="col-4 px-2">
              <img :src="dog.image ? 'http://localhost:3000' + dog.image : 'https://via.placeholder.com/150'" class="img-fluid rounded cursor-pointer img-thumbnail-custom active-thumb">
            </div>
            <div class="col-4 px-2">
              <img src="https://via.placeholder.com/150/f4ebf8/6a1b9a?text=+" class="img-fluid rounded img-thumbnail-custom">
            </div>
            <div class="col-4 px-2">
              <img src="https://via.placeholder.com/150/f4ebf8/6a1b9a?text=+" class="img-fluid rounded img-thumbnail-custom">
            </div>
        ...
        <div class="col-lg-8 pl-lg-5">
          ...
          <div class="price-box mb-4">
            <span class="font-weight-bold" style="font-size: 2.2rem; color: #4a148c;">
              {{ dog.price?.toLocaleString('vi-VN') }} vnđ
            </span>
          </div>

          <h5 class="section-title pb-2 mb-3">THÔNG TIN VỀ CÚN YÊU</h5>
          
          <div class="info-table-wrapper mb-4 rounded overflow-hidden shadow-sm" style="border: 1px solid #e1d5ed; max-width: 800px;">
            ...
            <div class="d-flex flex-wrap stripe-white">
              <div class="col-12 col-md-6 d-flex align-items-center py-2 px-3 border-right-md flex-nowrap">
                ...
                <a href="#" class="text-primary small font-weight-bold text-decoration-underline text-nowrap" 
        ...
        >
          <i
            class="fas"
            :class="dog.status === 'Đã duyệt' ? 'fa-hand-holding-usd' : 'fa-lock'"
          ></i>
          <span class="ml-2">
            {{
              dog.status === 'Đã duyệt'
            ...
            <a href="#" class="btn btn-chat font-weight-bold py-3 px-4 mr-3 mb-3 shadow-sm" style="border-radius: 8px;">
              <i class="fas fa-comment-dots mr-2"></i> CHAT VỚI TRẠI
            </a>
            <a href="tel:0379889868" class="btn btn-hotline font-weight-bold py-3 px-4 mb-3 shadow-sm" style="border-radius: 8px;">
    ...
    <div v-if="showVaccineModal" class="custom-modal-backdrop" @click.self="showVaccineModal = false">
      <div class="custom-modal-box shadow-lg bg-white rounded-lg overflow-hidden">
        <div class="bg-light p-3 d-flex justify-content-between align-items-center border-bottom">
          ...
          <button class="btn btn-sm btn-outline-secondary rounded-circle" style="width: 32px; height: 32px;" @click="showVaccineModal = false">
            <i class="fas fa-times"></i>
          </button>
        </div>
        <div class="p-4" style="max-height: 60vh; overflow-y: auto;">
          <div v-for="(v, i) in dog.healthRecord" :key="i" class="vaccine-badge shadow-sm bg-white mb-3">
            <i class="fas fa-shield-alt text-success mr-3 fa-2x"></i>
            <div class="d-flex flex-column">
              <strong class="text-dark" style="font-size: 1.1rem;">{{ v.vaccineId?.name || 'Đang cập nhật' }}</strong>
              <span class="vaccine-date mt-1"><i class="far fa-clock mr-1"></i> Ngày tiêm: {{ formatDate(v.dateInjected) }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-if="showDewormModal" class="custom-modal-backdrop" @click.self="showDewormModal = false">
      <div class="custom-modal-box shadow-lg bg-white rounded-lg overflow-hidden" style="max-width: 400px;">
        <div class="bg-light p-3 d-flex justify-content-between align-items-center border-bottom">
          ...
          <button class="btn btn-sm btn-outline-secondary rounded-circle" style="width: 32px; height: 32px;" @click="showDewormModal = false">
            <i class="fas fa-times"></i>
          </button>
        </div>
        <div class="p-4 text-center">
          ...
          <p class="mb-0 text-dark">Ngày tẩy giun gần nhất:<br><strong class="text-danger" style="font-size: 1.2rem;">{{ formatDate(dog.lastDeworming) }}</strong></p>
        </div>
      </div>
    </div>
...
export default {
  data() {
    return {
      ...
      showDewormModal: false, // Thêm biến cho modal Tẩy giun
    };
  },
  async created() {
...
/* BẢNG THÔNG SỐ: Đã gom gọn, padding vừa phải */
...
/* Ép nhãn chỉ chiếm 95px, giúp tiết kiệm không gian ngang */
.info-label { width: 95px; flex-shrink: 0; font-weight: 600; }

/* Flex-nowrap giúp Tẩy giun và Tiêm phòng luôn trên 1 dòng */
...
/* BỘ NÚT LIÊN HỆ TRÔI */
.floating-contact { position: fixed; bottom: 30px; left: 30px; display: flex; flex-direction: column; gap: 15px; z-index: 999; }
.float-btn { width: 50px; height: 50px; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: white; font-size: 1.5rem; text-decoration: none; transition: transform 0.2s; }
